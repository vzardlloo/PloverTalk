...
import cn.plover.service.UserService;
import cn.plover.utils.BaseServlet;
...
public class UserServlet extends BaseServlet {
	...
	public String sendMessage(HttpServletRequest req,HttpServletResponse resp) throws IOException{
		...
		String from = req.getParameter("from"); // 发言
		String face = req.getParameter("face"); // 表情
		String to = req.getParameter("to"); // 接收
	...
	public String kick(HttpServletRequest req,HttpServletResponse resp) throws IOException{
		...
		// 2.踢人:从userMap中将用户对应的session移除.
		// 获得userMap集合(在线列表)
		Map<User, HttpSession> userMap = (Map<User, HttpSession>) getServletContext()
				.getAttribute("userMap");
		// 获得这个用户对应的session.如何知道是哪个用户呢? id已经传�?�过�?.去数据库中查询?.
	...
	public String login(HttpServletRequest req,HttpServletResponse resp){
		...
		try {
			...
			if (existUser == null) {
				...
				// 使用监听器:HttpSessionBandingListener作用在JavaBean上的监听.
				...
				sourceMessage += "系统公告：<font color='red'>"
